...
| 外观设置 | 主题模式（跟随系统/浅色/深色）、动态取色开关、12 色主题色板、AMOLED 纯黑（对齐 ImageToolbox） |
...
2. **等待首次索引**：首次启动自动建立索引（耗时取决于文件数量，通常几十秒），之后每次打开都会自动增量更新，无变动时秒级完成；
3. **搜索**：输入关键词，文件名、目录名与文档全文实时命中，点击看详情，长按可多选删除。
